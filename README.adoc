...
When you commit your code modifications, Husky ensures that the following commands are launched by a pre-commit git hook:
...
The following commands are executed on push:

[source,javascript]
----
include::./index.js[tag=pre-push,indent=0]
----

So as long as the current branch name of your project does not reflect a work in progress, tests are executed on push.
...
`wip` is case-insensitive, `WIP` or `wIp` work as well.
====
